...
class ApiProvider {
    ...
    public func getSuggestedServices(completion: CompletionHandler<[BusSuggestion]> = nil) {
        ...
        let commonBusSuggestions = nearbyBusStops.flatMap { (busStops: [BusStop], event: EKEvent) -> [BusSuggestion] in
            ...
            let originBusServices = ApiProvider.shared.getBusStops(nearby: (LocationProvider.shared.currentLocation ?? K.mapView.defaultLocation).coordinate).flatMap { (busStop: BusStop) -> [(BusStop, BusService)] in
